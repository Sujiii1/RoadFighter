...
public class ObjectPoolingManager : MonoBehaviour
{
    ...
    public Queue<CarObject> ScoreUpObjectPool = new Queue<CarObject>(); // 아이템 풀 추가
    public Queue<CarObject> GotObjectPool = new Queue<CarObject>();     //무적 아이템
    ...
    private void InitializeItemPool()
    {
        ...
        for (int i = 0; i < itemPrefab.Length; i++)
        {
            ...
            for (int j = 0; j < 2; j++)
            {
                ...
                {
                    case 0:
                        ScoreUpObjectPool.Enqueue(newItem);
                        break;
                    case 1:
                        GotObjectPool.Enqueue(newItem);
                        break;
